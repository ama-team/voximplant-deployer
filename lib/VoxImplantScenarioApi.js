...
const SCENARIO_RETRIEVAL_ROUTE = '/GetScenarios';
const SCENARIO_CREATION_ROUTE = '/AddScenario';
const SCENARIO_UPDATE_ROUTE = '/SetScenarioInfo';
...
function VoxImplantScenarioApi(authentication, baseUrl) {
    ...
    this.create = function (name, content) {
        ...
        return executeRequest(METHOD_GET, SCENARIO_CREATION_ROUTE, {scenario_name: name, scenario_script: content})
            .then(function (result) {
    ...
    this.patch = function(name, content) {
        ...
        return executeRequest(METHOD_POST, SCENARIO_UPDATE_ROUTE, query)
    ...
    function scanScenarioPage(name, offset, size) {
        ...
        return executeRequest(METHOD_GET, SCENARIO_RETRIEVAL_ROUTE, {offset: offset, count: size})
            .then(function (response) {
                ...
                var matches = response.result.filter(function (scenario) { return scenario.scenario_name == name });
                if (matches.length > 0) {
    ...
    function getScenario(id) {
        ...
        return executeRequest(METHOD_GET, SCENARIO_RETRIEVAL_ROUTE, {scenario_id: id, with_script: true})
            .then(function (response) {
    ...
    function executeRequest(method, route, query) {
        ...
        return request({method: method, uri: uri, qs: query, json: true})
            .then(function (result) {
                if (result.error) {
                    throw {
